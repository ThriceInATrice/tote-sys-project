...
resource "aws_s3_object" "lambda_code" {
  ...
  source = "${path.root}/packages/ingestion_lambda/function.zip"
...
resource "aws_s3_object" "transform_lambda_code" {
  ...
  source = "${path.root}/packages/transform_lambda/function.zip"
...
resource "aws_s3_object" "layer_code" {
  ...
  source = "${path.root}/packages/layer_content.zip"
